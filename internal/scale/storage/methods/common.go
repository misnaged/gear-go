...
func (stor *Storage) getStorageRpc(gearRPC gear_rpc.IGearRPC) (string, error) {
	storKey, err := stor.getEncodedStorageKey()
	if err != nil {
		return "", fmt.Errorf(" gear.scale.StorageRequest failed: %v", err)
	}
	response, err := gearRPC.StateGetStorageLatest(storKey)
	if err != nil {
...
func (stor *Storage) DecodeStorage(gearRPC gear_rpc.IGearRPC, decodeData any) error {
	storageEncoded, err := stor.getStorageRpc(gearRPC)
	if err != nil {
