package gear_rpc_method

import "github.com/misnaged/gear-go/internal/models"

// gear_calculateGasForCreate
func (gearRPC *GearRpc) GearCalculateInitCreateGas(owner, codeId, payload string, value any, allowOtherPanic bool) (*models.RpcGenericResponse, error) {
	var params []any
	params = append(params, owner, codeId, payload, value, allowOtherPanic)
	return gearRPC.client.PostRequest(params, "gear_calculateGasForCreate")
}

// gear_calculateGasForHandle
func (gearRPC *GearRpc) GearCalculateHandleGas(owner, dest, payload string, value any, allowOtherPanic bool) (*models.RpcGenericResponse, error) {
	var params []any
	params = append(params, owner, dest, payload, value, allowOtherPanic)
	return gearRPC.client.PostRequest(params, "gear_calculateGasForHandle")
}
